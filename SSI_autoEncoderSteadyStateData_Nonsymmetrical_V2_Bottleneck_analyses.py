...
for indx in range(1,9):
    ...
    encoder = tensorflow.keras.layers.Dense(indx)(encoder)
    encoder = tensorflow.keras.layers.LeakyReLU(alpha=0.1)(encoder)
    distribution_mean = tensorflow.keras.layers.Dense(indx, name='mean')(encoder)
    distribution_variance = tensorflow.keras.layers.Dense(indx, name='log_variance')(encoder)
    ...
    ################### DECODER PART ############
    decoder_input = tensorflow.keras.layers.Input(shape=(indx)) # probably change the (6) to (2)!
    ...
    plt.plot(history.history['val_loss'], label='%s data' % indx)
    plt.legend()


plt.show()

    # exec(store_val_loss[indx]) = store_val_loss.append(history.history['val_loss'])
